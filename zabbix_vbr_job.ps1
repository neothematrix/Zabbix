...
# Close JSON object
	...
	"VmCount" {
		$type = (Get-VBRJob | Where-Object {$_.Id -like "*$ID*"}).JobType
		switch($type) {
			"Backup" { $query = Get-VBRBackup | Where-Object {$_.JobId -like "*$ID*"} }
			"Replica" { $query = Get-VBRReplica | Where-Object {$_.JobId -like "*$ID*"} }
